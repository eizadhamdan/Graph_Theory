...
/*
 * Implementation of the Ford-Fulkerson (FF) method with a DFS as a method of finding augmenting paths.
 * FF allows you to find the max flow through a directed graph as well as the min cut as a byproduct.
    ...
    public static int fordFulkerson(int[][] caps, int source, int sink) {
        ...
        for (int maxFlow = 0; ; ) {
            ...
            if (flow == 0) {
                ...
                // Uncomment to return the min-cut.
                // The nodes on the "LEFT SIDE" of the cut are marked as true and those on the "RIGHT SIDE" of the cut are marked as false.
                // This finds all the nodes which participated in the last (failed) attempt to find an augmenting path from the source to the sink in the DFS phase.
