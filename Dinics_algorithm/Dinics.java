 ...
 * The algorithm works by first constructing a level graph using a BFS and then finding augmenting paths on the level graph using multiple DFSs.
...
public class Dinics extends NetworkFlowSolverBase {
    ...
    public void solve() {
        ...
        // This is part of the Shimon Even and Alon Itai optimization of pruning deads ends as part of the DFS phase.
        int[] next = new int[n];
    ...
    // Do a BFS from source to sink and compute the depth/level of each node which is the minimum number of edges from that node to the source.
    private boolean bfs() {
        Arrays.fill(level, -1);
        level[s] = 0;
    ...
    private static void testSmallFlowGraph() {
        ...
        System.out.print("The maximum flow is: ");
        System.out.println(solver.getMaxFlow()); // 19
    }

    private static void anotherGraph() {
        int n = 11;
        int s = n - 1;
        int t = n - 2;
