...
/*
 * A recursive implementation of the TSP problem using dynamic programming.
 * The main idea is that since we need to do all n! permutations of nodes to find the optimal solution
 * then caching the results of sub paths can improve performance.
 *
    ...
    public TspDynamicProgrammingRecursive(int startNode, double[][] distance) {
        ...
        if (N > 32)
            throw new IllegalArgumentException(
                    "Matrix too large! A matrix that size for the DP TSP problem will have a time complexity of O(n^2*2^n)");
