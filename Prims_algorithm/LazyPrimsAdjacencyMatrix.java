...
/*
 * Implementation of the lazy Prim's algorithm with an adjacency matrix.
 * Upon visiting a new node we add all the edges to the min priority queue and also remove already seen edges when polling.
...
public class LazyPrimsAdjacencyMatrix {
    ...
    // Given an N*N undirected adjacency matrix, i.e. a graph with matrix[i][j] = matrix[j][i] for all i,j
    // this method finds the MST cost using Prim's algorithm
