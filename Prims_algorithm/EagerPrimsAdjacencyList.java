...
/*
 * Implementation of the eager version of Prim's algorithm which relies on using an indexed priority queue data structure to query the next best edge.
...
public class EagerPrimsAdjacencyList {
    ...
    // Returns the edges used in finding the minimum spanning tree, or returns null if no MST exists.
    public Edge[] getMst() {
        solve();
    ...
    private void solve() {
        ...
        // The degree of the d-ary heap supporting the IPQ can greatly impact performance, especially on dense graphs.
    ...
    private static class MinIndexedDHeap<T extends Comparable<T>> {
        ...
        // The Position Map (pm) maps Key Indexes (ki) to where the position of that key is represented in the priority queue in the domain [0, sz).
        public final int[] pm;

        // The Inverse Map (im) stores the indexes of the keys in the range [0, sz) which make up the priority queue.
        // It should be noted that 'im' and 'pm' are inverses of each other, so: pm[im[i]] = im[pm[i]] = i
        public final int[] im;
        ...
        // Recursively check if this heap is a min heap (to validate the heap invariant)
