...
/*
 * Implementation of the Floyd-Warshall algorithm to find all pairs of shortest paths between nodes in a graph.
 * We also demonstrate detect negative cycles and reconstruct the shortest path.
 *
...
public class FloydWarshallSolver {
     ...
     *
     * NOTE: Usually the diagonal of the adjacency matrix is all zeros (i.e. matrix[i][i] = 0 for all i)
     * since there is typically no cost to go from a node to itself, but this may depend on our graph and the problem we are trying to solve.
     */
    ...
    public void solve() {
        ...
        // Identify negative cycles by propagating the value 'NEGATIVE_INFINITY' to every edge that is part of or reaches into a negative cycle.
        for (int k = 0; k < n; k++)
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
     ...
     *
     * @return An array of nodes indexes of the shortest path from 'start' to 'end'. If 'start' and 'end' are not connected return an empty array.
     * If the shortest path from 'start' to 'end' are reachable by a negative cycle return -1.
     */
    ...
    // Create a graph with n nodes. The adjacency matrix is constructed such that the value of going from a node to itself is 0.
