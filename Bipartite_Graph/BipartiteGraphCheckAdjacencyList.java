...
/*
 * This file determines if a graph is bipartite or not. This can be achieved in linear time by coloring the visited nodes.
 * Time Complexity: O(V + E)
 */
import java.util.List;
//import Bipartite_Graph.Utils;
...
public class BipartiteGraphCheckAdjacencyList {
    ...
    // If the input graph is bipartite it has a two coloring which can be obtained through this method.
    // Each index in the returned array is either RED or BLACK indicating which color node i was colored.
    ...
    private void solve() {
        ...
        // The graph is not bipartite. Either not all the nodes were visited or the colorGraph method returned -1 meaning the graph is not 2-colorable.
        isBipartite = (nodesVisited == n);
        solved = true;
    }

    // Do a depth first search coloring the nodes of the graph as we go.
    // This method returns the count of the number of nodes visited while coloring the graph or -1 if this graph is not bipartite.
    private int colorGraph(int i, int color) {
        colors[i] = color;

        // Toggles the color between RED and BLACK by exploiting the binary representation of the constants and flipping the least significant bit on and off.
        ...
        for (int to : edges) {
            // Contradiction found. In a bipartite graph no two nodes of the same color can be next to each other!
            if (colors[to] == color) return -1;
            if (colors[to] == nextColor) continue;

            // If a contradiction is found propagate return -1 otherwise keep track of the number of visited nodes.
