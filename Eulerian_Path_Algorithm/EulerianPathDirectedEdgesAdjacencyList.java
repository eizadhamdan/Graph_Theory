package Eulerian_Path_Algorithm;

/*
 * Implementation of finding an Eulerian Path on a graph.
 * This implementation verifies that the input graph is fully connected and supports self loops and repeated edges between nodes.
 * Time Complexity: O(E)
 */
...
public class EulerianPathDirectedEdgesAdjacencyList {
    ...
    // Returns a list of edgeCount + 1 node id's that give the Eulerian path or null if no path exists or the graph is disconnected.
    public int[] getEulerianPath() {
        ...
        // It could be the case that the graph is disconnected in which case return null.
        if (path.size() != edgeCount + 1) return null;

        // Instead of returning the 'path' as a linked list return the solution as a primitive array for convenience.
